...
import type { SummarizeClickTimeSeries } from "@/types/tinybird";

export interface SummarizeClickTrendData {
  date: Date;
  dateString: string;
  clicks: number;
...
interface UseSummarizeClicksTimeseriesReturn {
...
/**
 * Hook to fetch summarize click time-series data
 ...
 */
export function useSummarizeClicksTimeseries(
  projectId: string | undefined,
  startDate?: Date,
  endDate?: Date,
  interval: TimeInterval = "day",
  isPublic: boolean = false
): UseSummarizeClicksTimeseriesReturn {
  const [data, setData] = useState<SummarizeClickTrendData[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<Error | null>(null);

  const fetchSummarizeClicksTrend = async () => {
    if (!projectId) {
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      const trendData = await queryAnalytics<SummarizeClickTimeSeries[]>(
      ...
      const dataMap = new Map<string, SummarizeClickTimeSeries>();
      ...
      const filledData: SummarizeClickTrendData[] = [];
  ...
  return {
    ...
    loading,
    error,
    refetch: fetchSummarizeClicksTrend,
  };
}
