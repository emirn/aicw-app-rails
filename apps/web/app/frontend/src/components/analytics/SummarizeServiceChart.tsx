...
import { getAIServiceFaviconUrl, getServiceColor } from "@/lib/ai-service-icons";
import {
  PieChart,
  Pie,
  Cell,
...
interface SummarizeServiceChartProps {
  data: SummarizeClickBreakdown[];
  loading?: boolean;
  error?: Error | null;
}

export function SummarizeServiceChart({
  data,
  loading,
  error,
}: SummarizeServiceChartProps) {
  if (loading) {
    return (
      <Card>
        <CardHeader>
    ...
    return (
      <Card>
        ...
        <CardContent className="flex flex-col items-center justify-center h-[300px] text-center">
          ...
          <p className="text-sm text-muted-foreground">
            No summarize click data available.
          </p>
        </CardContent>
      </Card>
    );
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <p className="text-sm text-muted-foreground">
          Distribution of clicks across AI services
        </p>
      </CardHeader>
      <CardContent>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  ...
                  if (active && payload && payload.length) {
                    ...
                    return (
                      <div className="bg-background border rounded-lg shadow-lg p-3">
                        ...
                        <p className="text-sm">
                          {data.value.toLocaleString()} clicks ({data.percentage.toFixed(1)}%)
                        </p>
          ...
          <div className="space-y-3">
            {data.map((service) => (
              <div
                key={service.aiService}
                className="flex items-center gap-3 p-3 rounded-lg border"
              >
                <img
                  src={getAIServiceFaviconUrl(service.aiService, 32)}
                  ...
                  onError={(e) => {
                    ...
                    fallback.style.backgroundColor = getServiceColor(service.aiService);
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {service.clickCount.toLocaleString()} clicks •{" "}
