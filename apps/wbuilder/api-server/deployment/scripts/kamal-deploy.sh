#!/bin/bash
#
# Deploy aicw-website-builder to AWS Lightsail using Kamal
...
echo ""
echo -e "${BLUE}======================================${NC}"
echo -e "${BLUE}AICW Website Builder - Kamal Deployment${NC}"
echo -e "${BLUE}======================================${NC}"
echo ""
...
# Read SSH private key content into AICW_WB_DEPLOY_SSH_PRIVATE_KEY
...
if ! ssh -i "$AICW_WB_DEPLOY_SSH_PRIVATE_KEY_PATH" "$AICW_WB_DEPLOY_USERNAME@$AICW_WB_SERVER_IP" "docker info" &> /dev/null; then
...
eval "$(ssh-agent -s)"
if ssh-add "$AICW_WB_DEPLOY_SSH_PRIVATE_KEY_PATH" 2>&1; then
...
echo "  - Build Docker image remotely on server"
...
if ! kamal deploy -c "$DEPLOYMENT_DIR/config/deploy.yml"; then
  ...
  echo "  3. Check container status: ssh $AICW_WB_DEPLOY_USERNAME@$AICW_WB_SERVER_IP 'docker ps -a'"
...
if curl -sf "https://$AICW_WB_DOMAIN/health" > /dev/null 2>&1; then
  echo -e "${GREEN}+ Health check passed${NC}"
  echo -e "${GREEN}+ API is running at https://$AICW_WB_DOMAIN${NC}"
else
