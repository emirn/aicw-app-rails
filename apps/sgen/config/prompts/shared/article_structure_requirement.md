...
```json
{
  ...
  "path": "string - URL-friendly path derived from title (required)",
...
- All required fields (id, path, title, description, keywords, content) MUST be present
- The path should be URL-friendly (lowercase, hyphens instead of spaces, no special characters)
