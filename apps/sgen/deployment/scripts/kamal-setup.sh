#!/bin/bash
#
# First-time server setup for sgen
...
echo ""
echo -e "${BLUE}======================================${NC}"
echo -e "${BLUE}Sgen Service - Server Setup${NC}"
echo -e "${BLUE}======================================${NC}"
echo ""
...
eval "$(ssh-agent -s)"
ssh-add "$SGEN_DEPLOY_SSH_PRIVATE_KEY_PATH"
...
echo ""
echo "Next steps:"
echo "  1. Ensure DNS A record exists: $SGEN_DOMAIN -> $SGEN_SERVER_IP"
